  ...
  *np->tf = *proc->tf;
  ...
  np->cwd = idup(proc->cwd);
	...
	if (strncmp(proc->name, "init", name_length) != 0 && strncmp(proc->name, "sh", name_length) != 0)
	{
		offset = 0;
		nread = 0;
		while ((nread = readExternalFile(proc, buf, offset, PGSIZE / 2)) != 0)
		{
			if (writeExternalFile(np, buf, offset, nread) == -1)
				panic("copying external file error!\n");
			offset += nread;
		}
	}
...
exit(void)
{
	...
	if (clearExternalFiles(proc) != 0) {
		panic("remove external files error");
	}
